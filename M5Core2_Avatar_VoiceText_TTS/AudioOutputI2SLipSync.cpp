...
AudioOutputI2SLipSync::AudioOutputI2SLipSync(int port, int output_mode, int dma_buf_count, int use_apll)
{
  this->portNo = port;
  this->i2sOn = false;
...
bool AudioOutputI2SLipSync::SetPinout()
{
  #ifdef ESP32
...
bool AudioOutputI2SLipSync::SetPinout(int bclk, int wclk, int dout)
{
  bclkPin = bclk;
  wclkPin = wclk;
  doutPin = dout;
...
AudioOutputI2SLipSync::AudioOutputI2SLipSync(long sampleRate, pin_size_t sck, pin_size_t data) {
    i2sOn = false;
    mono = false;
    bps = 16;
    channels = 2;
...
AudioOutputI2SLipSync::~AudioOutputI2SLipSync()
{
  #ifdef ESP32
    if (i2sOn) {
...
bool AudioOutputI2SLipSync::SetRate(int hz)
{
...
bool AudioOutputI2SLipSync::SetBitsPerSample(int bits)
{
...
bool AudioOutputI2SLipSync::SetChannels(int channels)
{
...
bool AudioOutputI2SLipSync::SetOutputModeMono(bool mono)
{
  this->mono = mono;
  return true;
}

bool AudioOutputI2SLipSync::begin(bool txDAC)
{
  #ifdef ESP32
    if (!i2sOn)
    {
...
int AudioOutputI2SLipSync::getLevel()
{
  return aout_level;
}

void AudioOutputI2SLipSync::setLevel(int level)
{
	aout_level = level;
}

bool AudioOutputI2SLipSync::ConsumeSample(int16_t sample[2])
{
...
void AudioOutputI2SLipSync::flush()
{
  #ifdef ESP32
...
bool AudioOutputI2SLipSync::stop()
{
  if (!i2sOn)
    return false;
